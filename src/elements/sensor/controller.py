...
class SensorController(Sofa.Core.Controller):
    ...
    def initTrajectoryByKeyPoint(self):
        ...
        time = 0
        self.tableKeyPoints.append(
            [0, 0, 0, 0]
        )  # Time dy theta_x theta_z WARNING = Suppose time ordering
    ...
    def defineTrajectoryByKeyPoint(self, time):
        ...
        interpolFactor = (self.tableKeyPoints[self.currentKey + 1][0] - time) / (
            self.tableKeyPoints[self.currentKey + 1][0]
            - self.tableKeyPoints[self.currentKey][0]
        )

        self.dy = (
            interpolFactor * self.tableKeyPoints[self.currentKey][1]
            + (1 - interpolFactor) * self.tableKeyPoints[self.currentKey + 1][1]
        )
        self.theta_x = (
            interpolFactor * self.tableKeyPoints[self.currentKey][2]
            + (1 - interpolFactor) * self.tableKeyPoints[self.currentKey + 1][2]
        )
        self.theta_z = (
            interpolFactor * self.tableKeyPoints[self.currentKey][3]
            + (1 - interpolFactor) * self.tableKeyPoints[self.currentKey + 1][3]
        )
    ...
    def onAnimateBeginEvent(self, e):
        ...
        if self.activated:
            ...
            pos = [0.0, self.y + self.dy, 0.0]
            quat = Quat.createFromEuler([self.theta_x, 0.0, self.theta_z])
            self.mo.rest_position = [
